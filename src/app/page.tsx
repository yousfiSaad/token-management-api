...
export default function Home() {
  ...
  const [tokens, setTokens] = useState<CreateTokenResponse[]>([]);
  ...
  const handleListTokens = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `/api/tokens?userId=${encodeURIComponent(listUserId)}`,
        {
          headers: {
            "X-API-Key": apiKey,
          },
        ...
        {createdToken && (
          <div className="mt-4 bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-md">
            ...
            <div className="space-y-1 text-sm">
              <p>
                <strong>ID:</strong> {createdToken.id}
              </p>
              <p>
                <strong>User ID:</strong> {createdToken.userId}
              </p>
              <p>
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        {tokens.length > 0 && (
          <div className="mt-6">
            <h3 className="text-lg font-medium text-gray-800 mb-4">
              Non-Expired Tokens ({tokens.length})
            </h3>
        ...
        {tokens.length === 0 && listUserId && !isListing && !listError && (
          <p className="mt-6 text-gray-500 italic">
            No non-expired tokens found for this user.
          </p>
        )}
      </div>
    </main>
